...
# @scaffold:process_id gate-probe-readonly
# @scaffold:process_name "Gate Probe Read Only"
# @scaffold:file_hash sha256:5c7a2981f439f87b2c1ad01fb5be90283e255a2efcab803fb4ce9dacd9376828
#
# PURPOSE: Temporal workflow that orchestrates the Gate Probe Read Only process.
#
...
"""
Gate Probe Read Only Workflow

Temporal workflow that orchestrates the Gate Probe Read Only process.
...
from processes.fixture_tenant.gate_probe_read_only.c_gpreadon.constants import (
...
with workflow.unsafe.imports_passed_through():
    from processes.fixture_tenant.gate_probe_read_only.c_gpreadon.activities import (
    ...
    from processes.fixture_tenant.gate_probe_read_only.c_gpreadon.models.artifacts import (
        HumanTaskResult,
...
# VERSION CONSTANTS
# =============================================================================
PROCESS_CHANGE_ID = "gpreadonly1"  # Full change UUID
PLATFORM_PATCH_VERSION = 17



@dataclass
class GateProbeReadOnly_gpreadonInput:
    """Input for the Gate Probe Read Only workflow (version gpreadon)."""
...
@dataclass
class GateProbeReadOnly_gpreadonOutput:
    """Output from the Gate Probe Read Only workflow (version gpreadon)."""
...
@workflow.defn(name="gate_probe_read_only_gpreadonly1")
class GateProbeReadOnly_gpreadonWorkflow:
    """
    Gate Probe Read Only workflow with signal-based human tasks.
    ...
    async def _check_paused(self, input: "GateProbeReadOnly_gpreadonInput") -> None:
    ...
    async def _handle_activity_failure(
        ...
        cause: BaseException,
        on_failure: str,
        input: "GateProbeReadOnly_gpreadonInput",
    ...
    async def _execute_step_resolve(
        ...
        activity_input = ResolveReadWorkspaceStepresoInput(
            ...
            step_lineage_id=STEP_RESOLVE_READ_WORKSPACE,
            tenant_code=input.tenant_code,
            repo_key=Artifact.wrap(str(workflow.uuid4()), "petclinic"),
            branch_base=Artifact.wrap(str(workflow.uuid4()), "main"),
    ...
    async def _execute_step_branch(
        ...
        activity_input = CreateFeatureBranchStepbranInput(
            ...
            context_id=Artifact.wrap(str(workflow.uuid4()), "probe-ro1"),
    ...
    async def _execute_step_code(
        ...
        activity_input = GateProbeCodeStepcodeInput(
            ...
            step_lineage_id=STEP_GATE_PROBE_CODE,
            tenant_code=input.tenant_code,
            task=Artifact.wrap(str(workflow.uuid4()), "List the files at the repository root with Glob and stop by calling complete_step with a one-sentence summary. Do not write or change anything."),
    ...
    @workflow.run
    async def run(self, input: GateProbeReadOnly_gpreadonInput) -> GateProbeReadOnly_gpreadonOutput:
        """Execute the Gate Probe Read Only workflow."""
        workflow_id = workflow.info().workflow_id

        workflow.logger.info(
            "Starting GateProbeReadOnly_gpreadonWorkflow",
            extra={
                "instance_id": input.instance_id,
                "process_def_id": input.process_def_id,
        ...
        try:
            ...
            return GateProbeReadOnly_gpreadonOutput(
                success=True,
                outcome="completed",
            )
