...
# @scaffold:file_hash sha256:ccf4ff2f511ee04fa4eceeea0fff6b40a5a7b3b2c0402fee5cd1aeea5d7376e0
#
...
from processes.fixture_tenant.gate_probe_flip.c_gpflip1.constants import STEP_GATE_PROBE_CODE
...
@activity.defn(name="gate_probe_code_stepcode_c_gpflip1")
async def gate_probe_code_stepcode(
    ...
    async with platform.for_tenant(input.tenant_code) as p:
        ...
        if _expr_val_task is not None:
            ...
            expressions["task"] = "Do exactly these three things and nothing else. 1) Write a file named GATE_PROBE.md at the repository root containing the single line: gate probe. 2) Commit only that file with the message: gate probe. 3) Immediately call the complete_step tool with a one-sentence summary and stop."
