...
# @scaffold:file_hash sha256:37ade01e00a3adaa49b2dfe95078e04fc521492a2b4a74db5bfa92272dedbe87
#
# PURPOSE: Temporal workflow that orchestrates the Gate Probe Flip process.
#
...
"""
Gate Probe Flip Workflow

Temporal workflow that orchestrates the Gate Probe Flip process.
...
from processes.fixture_tenant.gate_probe_flip.c_gpflip1.constants import (
...
with workflow.unsafe.imports_passed_through():
    from processes.fixture_tenant.gate_probe_flip.c_gpflip1.activities import (
    ...
    from processes.fixture_tenant.gate_probe_flip.c_gpflip1.models.artifacts import (
        HumanTaskResult,
...
# VERSION CONSTANTS
# =============================================================================
PROCESS_CHANGE_ID = "gpflip1"  # Full change UUID
PLATFORM_PATCH_VERSION = 17



@dataclass
class GateProbeFlip_gpflip1Input:
    """Input for the Gate Probe Flip workflow (version gpflip1)."""
...
@dataclass
class GateProbeFlip_gpflip1Output:
    """Output from the Gate Probe Flip workflow (version gpflip1)."""
...
@workflow.defn(name="gate_probe_flip_gpflip1")
class GateProbeFlip_gpflip1Workflow:
    """
    Gate Probe Flip workflow with signal-based human tasks.
    ...
    async def _check_paused(self, input: "GateProbeFlip_gpflip1Input") -> None:
    ...
    async def _handle_activity_failure(
        ...
        cause: BaseException,
        on_failure: str,
        input: "GateProbeFlip_gpflip1Input",
    ...
    async def _execute_step_resolve(
        ...
        activity_input = ResolveReadWorkspaceStepresoInput(
            ...
            step_lineage_id=STEP_RESOLVE_READ_WORKSPACE,
            tenant_code=input.tenant_code,
            repo_key=Artifact.wrap(str(workflow.uuid4()), "flip-probe"),
    ...
    async def _execute_step_branch(
        ...
        activity_input = CreateFeatureBranchStepbranInput(
            ...
            context_id=Artifact.wrap(str(workflow.uuid4()), "probe-flip1"),
    ...
    @workflow.run
    async def run(self, input: GateProbeFlip_gpflip1Input) -> GateProbeFlip_gpflip1Output:
        """Execute the Gate Probe Flip workflow."""
        workflow_id = workflow.info().workflow_id

        workflow.logger.info(
            "Starting GateProbeFlip_gpflip1Workflow",
            extra={
                "instance_id": input.instance_id,
                "process_def_id": input.process_def_id,
        ...
        try:
            ...
            return GateProbeFlip_gpflip1Output(
                success=True,
                outcome="completed",
            )
