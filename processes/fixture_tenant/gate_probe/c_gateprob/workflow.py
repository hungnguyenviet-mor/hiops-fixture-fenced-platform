...
# @scaffold:process_id gate-probe
# @scaffold:process_name "Gate Probe"
# @scaffold:file_hash sha256:46469b184e3fe61c9c8ad36dd7054d16e708d52f437c52f8b723ca6fc37d280d
#
# PURPOSE: Temporal workflow that orchestrates the Gate Probe process.
#
...
Temporal workflow that orchestrates the Gate Probe process.
...
from processes.fixture_tenant.gate_probe.c_gateprob.constants import (
...
with workflow.unsafe.imports_passed_through():
    from processes.fixture_tenant.gate_probe.c_gateprob.activities import (
...
# VERSION CONSTANTS
# =============================================================================
PROCESS_CHANGE_ID = "gateprobe1"  # Full change UUID
PLATFORM_PATCH_VERSION = 17



@dataclass
class GateProbe_gateprobInput:
    """Input for the Gate Probe workflow (version gateprob)."""
...
@dataclass
class GateProbe_gateprobOutput:
    """Output from the Gate Probe workflow (version gateprob)."""
...
@workflow.defn(name="gate_probe_gateprobe1")
class GateProbe_gateprobWorkflow:
    """
    Gate Probe workflow with signal-based human tasks.
    ...
    async def _check_paused(self, input: "GateProbe_gateprobInput") -> None:
    ...
    async def _execute_step_branch(
        ...
        activity_input = CreateFeatureBranchStepbranInput(
            ...
            context_id=Artifact.wrap(str(workflow.uuid4()), "probe-001"),
    ...
    @workflow.run
    async def run(self, input: GateProbe_gateprobInput) -> GateProbe_gateprobOutput:
        """Execute the Gate Probe workflow."""
        workflow_id = workflow.info().workflow_id

        workflow.logger.info(
            "Starting GateProbe_gateprobWorkflow",
            extra={
                "instance_id": input.instance_id,
                "process_def_id": input.process_def_id,
        ...
        try:
            ...
            return GateProbe_gateprobOutput(
                success=True,
                outcome="completed",
            )
